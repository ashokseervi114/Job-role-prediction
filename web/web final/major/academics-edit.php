...
// Getting the usersdetail Table Primary Key which is also a foriegn in academics Table.
	...
	if(isset($_POST['saveAcadDetails'])) {
		...
		if(!$acad_detail_Id) {
			...
			$acaded = Acad::find_by_contents($acad_detail_Id_content, $acad_detail_Id);
			
			$acaded->operating_system = $os;
			$acaded->linux_administration = $linux;
			$acaded->java = $java;
			$acaded->cpp = $cpp;
			$acaded->c = $pinc;
			$acaded->python = $python;
			$acaded->web_programming = $webprog;
			$acaded->network = $network;
			...
			<form method="post">
				<div class="form-row">
					<div class="form-group col-md-6">
						...
						<input type="number" class="form-control" name="opsys" id="operatingSystem" placeholder="" value="<?php echo $ad->operating_system; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="linuxAd">Academic Percentage in Linux Administration</label>
						<input type="number" class="form-control" name="linuxad" id="linuxAd" placeholder="" value="<?php echo $ad->linux_administration; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="javaBeginner">Academic Percentage in Java Programming</label>
						<input type="number" class="form-control" name="java" id="linux-ad" placeholder="" value="<?php echo $ad->java; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="cpp">Academic Percentage in C++</label>
						<input type="number" class="form-control" name="cpp" id="cpp" placeholder="" value="<?php echo $ad->cpp; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="progInC">Academic Percentage in C</label>
						<input type="number" class="form-control" name="pic" id="progInC" placeholder="" value="<?php echo $ad->c; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="python">Academic Percentage in Python</label>
						<input type="number" class="form-control" name="python" id="python" placeholder="" value="<?php echo $ad->python; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="webprog">Academic Percentage in Web Programming</label>
						<input type="number" class="form-control" name="webprog" id="webprog" placeholder="" value="<?php echo $ad->web_programming; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="network">Academic Percentage in Network</label>
						<input type="number" class="form-control" name="network" id="network" placeholder="" value="<?php echo $ad->network; ?>">
					</div>
