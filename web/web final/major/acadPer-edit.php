...
// Getting the usersdetail Table Primary Key which is also a foriegn in academics Table.
	...
	if(isset($_POST['saveAcadPerDetails'])) {
		...
		if(!$acad_detail_Id) {
			...
			$acaded = AcadPer::find_by_contents($acad_detail_Id_content, $acad_detail_Id);
			
			
			$acaded->operating_system_per = $os;
			$acaded->algorithm_per = $algorithm;
			$acaded->programming_concept_per = $progConcept;
			$acaded->software_eng_per = $softEngineering;
			$acaded->computer_net_per = $comNetwork;
			$acaded->electronics_per = $electSub;
			$acaded->computer_arch_per = $compArch;
			$acaded->mathematics_per = $maths;
			$acaded->communication_per = $commSkill;
			...
			<form method="post">
				<div class="form-row">
					<div class="form-group col-md-6">
						...
						<input type="number" class="form-control" id="operatingSystem" placeholder="" name="operatingSystem" value="<?php echo $ad->operating_system_per; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="algorithm">Academic Percentage in Algorithm</label>
						<input type="number" class="form-control" id="algorithm" placeholder="" value="<?php echo $ad->algorithm_per; ?>" name="algorithm">
					</div>
					<div class="form-group col-md-6">
						<label for="progConcept">Academic Percentage in Programming Concept</label>
						<input type="number" class="form-control" id="progConcept" placeholder="" value="<?php echo $ad->programming_concept_per; ?>" name="progConcept">
					</div>
					<div class="form-group col-md-6">
						<label for="softEngineering">Academic Percentage Software Engineering</label>
						<input type="number" name="softEngineering" class="form-control" id="softEngineering" placeholder="" value="<?php echo $ad->software_eng_per; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="comNetwork">Academic Percentage in Computer Network</label>
						<input type="number" name="comNetwork" class="form-control" id="comNetwork" placeholder="" value="<?php echo $ad->computer_net_per; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="electSub">Academic Percentage in Electronics Subject</label>
						<input type="number" name="electSub" class="form-control" id="electSub" placeholder="" value="<?php echo $ad->electronics_per; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="compArch">Academic Percentage in Computer Architecture</label>
						<input type="number" name="compArch" class="form-control" id="compArch" placeholder="" value="<?php echo $ad->computer_arch_per; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="maths">Academic Percentage in Mathematics</label>
						<input type="number" name="maths" class="form-control" id="maths" placeholder="" value="<?php echo $ad->mathematics_per; ?>">
					</div>
					<div class="form-group col-md-6">
						<label for="commSkill">Academic Percentage in Communication Skills</label>
						<input type="number" name="commSkill" class="form-control" id="commSkill" placeholder="" value="<?php echo $ad->communication_per; ?>">
					</div>
				</div>
